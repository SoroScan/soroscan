        ...
        >
          <div className="flex justify-between items-start">
            <div>
              ...
              <div className="font-mono text-terminal-cyan text-sm">{contract.contractId.slice(0, 8)}...</div>
            </div>
            <span
            ...
            >
              <span
                className={`w-2 h-2 rounded-full ${
                  contract.status === "active" ? "bg-terminal-green animate-pulse" : "bg-terminal-gray"
                }`}
              />
          ...
          {contract.tags && contract.tags.length > 0 && (
            <div className="flex gap-1 flex-wrap">
              ...
              {(contract.tags?.length ?? 0) > 3 && <span className="text-xs text-terminal-gray">+{(contract.tags?.length ?? 0) - 3}</span>}
            </div>
          )}
        </div>
      ))}
        ...
        <TableBody>
          {contracts.map((contract) => (
            <TableRow key={contract.id} onClick={() => handleRowClick(contract.id)} className="cursor-pointer">
              <TableCell className="font-mono text-terminal-cyan">{contract.contractId.slice(0, 8)}...</TableCell>
              <TableCell className="font-semibold">{contract.name}</TableCell>
              <TableCell>
                <span
                  className={`inline-flex items-center gap-2 px-2 py-1 text-xs font-mono ${
                    contract.status === "active" ? "text-terminal-green border border-terminal-green/30 bg-terminal-green/10" : "text-terminal-gray border border-terminal-gray/30 bg-terminal-gray/10"
                  }`}
                >
                  <span className={`w-2 h-2 rounded-full ${contract.status === "active" ? "bg-terminal-green animate-pulse" : "bg-terminal-gray"}`} />
                  {contract.status.toUpperCase()}
                </span>
              </TableCell>
              ...
              <TableCell>
                <div className="flex gap-1 flex-wrap">
                  {contract.tags?.slice(0, 3).map((tag) => (
                    <span key={tag} className="text-xs px-2 py-0.5 bg-terminal-cyan/10 text-terminal-cyan border border-terminal-cyan/30">
                      {tag}
                    </span>
                  ))}
                  {(contract.tags?.length ?? 0) > 3 && <span className="text-xs text-terminal-gray">+{(contract.tags?.length ?? 0) - 3}</span>}
