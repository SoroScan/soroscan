...
const VARIANT_STYLES: Record<ProgressVariant, { fill: string; track: string }> =
  {
    success: {
...
export function ProgressBar({
  ...
  const displayLabel =
    label ?? (showPercentage && !indeterminate ? percentage : null)
    ...
    <div className={cn("w-full space-y-2 font-terminal-mono", className)} {...props}>
      {labelPosition === "above" && displayLabel ? (
        <div className="flex items-center justify-between gap-3 text-xs tracking-widest text-terminal-gray">
          ...
          {showPercentage && !indeterminate ? (
            <span className="tabular-nums text-terminal-green">{percentage}</span>
          ) : null}
        </div>
      ) : null}
      ...
      >
        <div
          ...
          className={cn(
            "absolute inset-y-0 left-0 flex items-center justify-between px-2 text-[10px] transition-[width,transform] duration-500 ease-out",
            styles.fill,
            indeterminate
