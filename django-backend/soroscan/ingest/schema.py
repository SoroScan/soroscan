...
class CallGraphType:
    last_computed: auto
    has_cycles: auto
    cyclic_dependencies: strawberry.scalars.JSON
    total_nodes: auto
    total_edges: auto
    critical_paths: strawberry.scalars.JSON
...
class Query:
    ...
    @strawberry.field
    def dependencies_for_contract(self, contract_id: str) -> list[ContractDependencyType]:
        """Get all dependencies (calls and called_by) for a specific contract."""
        from django.db.models import Q
        return list(ContractDependency.objects.filter(
            Q(caller__contract_id=contract_id) | Q(callee__contract_id=contract_id)
        ).select_related("caller", "callee"))

    @strawberry.field
    def call_graph(self) -> Optional[CallGraphType]:
        """Get the latest global call graph analysis."""
        return CallGraph.objects.first()
